              ...
              if (tokenIsValid) {
                ...
                _data.update("checks", id, checkData, (err) => {
                  if (!err) {
                    ...
                    callback(500, {
                      Error: "Unable to update the checks object",
                    });
                  }
                });
              } else {
...
handlers._checks.delete = (data, callback) => {
  const id =
    typeof data.queryStringObject.id === "string" &&
    data.queryStringObject.id.trim().lenght === 20
      ? data.queryStringObject.id
      : false;
  ...
  if (id) {
    _data.read("checks", id, (err, checkData) => {
      if (!err && checkData) {
            ...
            if (tokenIsValid) {
              _data.delete("checks", id, (err) => {
                if (!err) {
                  _data.read("users", checkData.userPhone, (err, userData) => {
                    if (!err && userData) {
                      ...
                      if (checkPosition > -1) {
                        ...
                        _data.update(
                          ...
                          (err) => {
                            if (!err) {
                              ...
                              callback(500, {
                                Error: "Cound't delete the specified user",
                              });
                            }
                          },
                        );
                        ...
                        callback(500, {
                          Error: "Could not find the checks in the user",
                        });
                      }
                    } else {
                      callback(500, {
                  ...
                  callback(500, {
                    Error: "Unable to delete the checks object",
                  });
                }
              });
            } else {
        ...
        callback(400, { Error: "Unable to fetch checkata" });
      }
    });
  } else {
    callback(400, { Error: "Invalid id " });
